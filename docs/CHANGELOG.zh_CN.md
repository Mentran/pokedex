...
- 首页长按确定打开音量亮度，上下调节，短按切换，长按返回；设置写入 NVS。叫声仍比 BGM 高 20。
- 开机去掉浅绿色地面椭圆，只留角色脚下的深色落点影。第二句图鉴仍跟介绍放在同一页，不单独成页。
- 猜猜我是谁：每轮随机给剪影或图鉴介绍句，确定直接揭晓图鉴，不给选项。大木讲堂人像改成上半身裁切，80px 放在左下或右下角。开机只播耿鬼对尼多力诺，约 4 秒，场地跟图鉴绿屏同色，进首页后再起 BGM。循环播放「我是谁」同款 8-bit 宝可梦中心 BGM，音量 40，叫声 60。
- 大木讲堂世界小知识扩充到 224 条。
- 删掉已不再使用的 16×16 像素固件资源和本地预览图。
- 开机宝可梦改用封面同一套 80×80 官方立绘；大木讲堂人像改为大木 oak-gen3 + Let's Go 小智/小刚/小霞/小茂，打进 `trainers.bin`。
- 开机全屏随机播放五条短动画，任意键跳过；「宝可梦小知识」改成「大木讲堂」，进入和翻条时换人像。
- 第一世代图鉴改成红色手持外壳，封面 80×80 立绘，叫声为 IMA-ADPCM，放进 `pokedexfs`。
  已写身份的胸牌用分段 `idf.py flash`，不要从 `0x0` 整包刷合并镜像。
- 能力值页增加六项总和；招式页列出全部初代升级自学招式；新增克制页，显示 2 倍 / 0.5 倍 / 0 的攻击和防御属性。
- 封面增加身高、体重和性别比，捕获率不显示；介绍页在第二句与第一句不重复时显示 trivia；能力页写成「特性：」并附介绍；克制页被打和打出同页，太长时上下键滚动；小知识是首页独立入口，至少 100 条，不显示第几条。招式威力和招式说明不加。封面双击确定放大立绘，短按确定仍进介绍。
