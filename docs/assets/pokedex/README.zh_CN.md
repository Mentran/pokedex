...
开机直接进首页，三个入口。这套玩法用红色手持图鉴外壳（转轴、内屏、喇叭孔），不用模板的天空 / 草地 / 吉祥物。电量放在红色顶框右上角。
...
随机模式从首页进入时抽一个新编号。之后上下键仍按图鉴顺序走。小知识进入时随机一条，之后上下键在池子里翻。
...
**封面：** 编号、80 × 80 图、中文名、英文名、一或两个属性标签、分类、身高体重和性别比。打开封面会播叫声。
...
首页文案是「宝可梦图鉴 / 第一世代151只」，三个入口为图鉴浏览、随机遇见、宝可梦小知识。小知识页不显示条数。
