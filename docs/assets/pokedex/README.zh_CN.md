...
| 特性 | PokeAPI abilities 的 `zh-hans` 名 | 初代没有特性，这里展示现代特性 |
| 能力值 | HP / 攻击 / 防御 / 特攻 / 特防 / 速度 | 画成条 |
| 招式 | 红绿版（或 generation-red）升级招式，最多 8 个 | 超过 8 个时优先留下后期标志招 |
...
开机直接进首页，两个模式。这套玩法用红色手持图鉴外壳（转轴、内屏、喇叭孔），不用模板的天空 / 草地 / 吉祥物。电量放在红色顶框右上角。

```text
首页
  顺序查看  ----+
  随机查看  ----+--> 条目（总是先封面）
                  上 / 下 : 上一只 / 下一只（顺序模式在 1..151 循环；
                            随机模式进入后也可以继续翻，避免卡在一只）
                  确定短按 : 封面 -> 介绍 -> 能力 -> 招式 -> 克制 -> 进化 -> 封面
                  确定长按 : 在封面播放叫声，然后回首页
```

随机模式从首页进入时抽一个新编号。之后上下键仍按图鉴顺序走。
...
**封面：** 编号、80 × 80 图、中文名、英文名、一或两个属性标签、分类。打开封面会播叫声。

**介绍：** 分类加一段折行正文。若有第二句且不重复，接在后面。

**能力：** 六条能力条，1–255 映射到约 100 px 轨道，并显示六项总和。

**招式：** 全部初代升级自学招式，带学会等级。

**克制：** 防御 4 倍 / 2 倍 / 1/2 / 1/4 / 0，以及攻击 2 倍 / 1/2 / 0，按现代属性表计算。
